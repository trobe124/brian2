...
print('The time needed to load training set:', end - start)
 
start = time.time()
testing = get_labeled_data(MNIST_data_path + 'testing', bTrain = False)
end = time.time()
print('The time needed to load test set:', end - start)


#------------------------------------------------------------------------------ 
# set parameters and equations
#------------------------------------------------------------------------------
test_mode = True # When retraining, this must be changed to False
...
rate_monitors['Xe'] = b.PopulationRateMonitor(input_groups['Xe'])
...
if rate_monitors:
    ...
    for i, name in enumerate(rate_monitors):
        b.subplot(len(rate_monitors), 1, 1+i)
        b.plot(rate_monitors[name].t/b.second, rate_monitors[name].rate, '.')
...
if spike_monitors:
    ...
    for i, name in enumerate(spike_monitors):
        b.subplot(len(spike_monitors), 1, 1+i)
        b.plot(spike_monitors[name].t/b.ms, spike_monitors[name].i, '.')
...
if spike_counters:
    ...
    for i, name in enumerate(spike_counters):
        b.subplot(len(spike_counters), 1, 1+i)
